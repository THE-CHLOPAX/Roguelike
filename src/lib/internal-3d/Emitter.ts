...
export class Emitter<T> {
  ...
  public toggleEvent(event: keyof T, enabled: boolean): void {
    ...
    if (enabled) {
      this._suppressedListeners = this._suppressedListeners.filter(
  ...
  public trigger<U extends keyof T>(event: U, payload?: T[U]): void {
    ...
    const callback = (listener: Listener<keyof T, T[keyof T]>) =>
      listener.cb(payload as T[keyof T]);

    this._listeners.filter((listener) => listener.event === event).forEach(callback);
  }
}
