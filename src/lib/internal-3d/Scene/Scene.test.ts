...
describe('Scene base class', () => {
  ...
  it('should convert objects with gameObjectComponents userData to GameObject instances', async () => {
    ...
    const sphereChildCamera = sphereChild!.children.find(
      (child) => child.name === 'PerspectiveCamera'
    );
    ...
    // Non-game object lights
    const directionalLightChild = testScene.children.find(
      (child) => child.name === 'DirectionalLight'
    );
    ...
    const torusChildGameObject = torusChild!.children.find(
      (child) => child.name === 'TorusGameObjectChild'
    );
    expect(torusChildGameObject).toBeDefined();
    expect(torusChildGameObject).toBeInstanceOf(GameObject);

    const torusChildRegular = torusChild!.children.find(
      (child) => child.name === 'TorusRegularChild'
    );
    ...
    const torusGrandGameObject = torusChildRegular!.children.find(
      (child) => child.name === 'TorusRegularChildGameObjectChild'
    );
