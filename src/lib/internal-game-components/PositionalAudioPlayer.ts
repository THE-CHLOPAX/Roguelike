...
import { logger, GameObject, SoundChannel, useSoundsStore, GameObjectEventMap } from '@tgdf';
...
> extends GameObjectComponent<PositionalAudioPlayerOptions, T> {
  ...
  constructor(gameObject: GameObject<T>, options: PositionalAudioPlayerOptions = {}) {
  ...
  public async loadAudio(audioElement: HTMLAudioElement): Promise<void> {
    return new Promise((resolve, reject) => {
      this._loadAudioBuffer(audioElement)
        .then(() => {
          resolve();
        })
        .catch((error) => {
  ...
  public play(
    ...
    if (!this._bufferCache.has(audioElement.src)) {
      logger({
        message: `PositionalAudioPlayer: No audio buffer
        available for element src: ${audioElement.src}`,
        type: 'warn',
      });
    }
