...
describe('InputSequenceTracker', () => {
  it('returns null while the sequence is incomplete', () => {
    ...
    expect(tracker.push(ACTION_UP, 0, [skill])).toBeNull();
    expect(tracker.push(ACTION_DOWN, 100, [skill])).toBeNull();
  });
  ...
  it('returns the skill once its full sequence is entered in order', () => {
    ...
    tracker.push(ACTION_UP, 0, [skill]);
    tracker.push(ACTION_DOWN, 100, [skill]);

    expect(tracker.push(ACTION_LEFT, 200, [skill])).toBe(skill);
  });
  ...
  it('returns null when the inputs do not match any skill sequence', () => {
    ...
    expect(tracker.push(ACTION_DOWN, 0, [skill])).toBeNull();
    expect(tracker.push(ACTION_UP, 100, [skill])).toBeNull();
  });
  ...
  it('matches the skill corresponding to the entered sequence among multiple skills', () => {
    ...
    tracker.push(ACTION_DOWN, 0, skills);

    expect(tracker.push(ACTION_RIGHT, 100, skills)).toBe(healSkill);
  });
  ...
  it('matches when every input arrives within the timeout of the previous one', () => {
    ...
    tracker.push(ACTION_UP, 0, [skill]);
    tracker.push(ACTION_DOWN, TIMEOUT_MS - 1, [skill]);

    expect(tracker.push(ACTION_LEFT, (TIMEOUT_MS - 1) * 2, [skill])).toBe(skill);
  });
  ...
  it('accepts an input arriving exactly at the timeout boundary', () => {
    ...
    expect(tracker.push(ACTION_DOWN, TIMEOUT_MS, [skill])).toBe(skill);
  });
  ...
  it('resets progress and discards the input when it arrives after the timeout', () => {
    ...
    expect(tracker.push(ACTION_UP, TIMEOUT_MS + 1, [skill])).toBeNull();
    // The late input was discarded, so ACTION_DOWN alone must not complete the sequence.
    expect(tracker.push(ACTION_DOWN, TIMEOUT_MS + 2, [skill])).toBeNull();
  });
  ...
  it('matches a fresh sequence entered after a timeout reset', () => {
    ...
    tracker.push(ACTION_UP, 0, [skill]);
    tracker.push(ACTION_LEFT, TIMEOUT_MS + 100, [skill]);

    tracker.push(ACTION_UP, TIMEOUT_MS + 200, [skill]);

    expect(tracker.push(ACTION_DOWN, TIMEOUT_MS + 300, [skill])).toBe(skill);
  });
  ...
  it('resets after a match', () => {
    ...
    tracker.push(ACTION_UP, 0, [skill]);
    expect(tracker.push(ACTION_DOWN, 100, [skill])).toBe(skill);
